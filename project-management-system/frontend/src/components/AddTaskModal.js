...
const AddTaskModal = ({ isAddTaskModalOpen, setAddTaskModal, projectId = null, taskId = null, edit = false, refreshData }) => {
    ...
    useEffect(() => {
        if (edit && isAddTaskModalOpen) {
            axios.get(`${API_BASE_URL}/project/${projectId}/task/${taskId}`)
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post(`${API_BASE_URL}/project/${projectId}/task`, { title, description: desc })
                .then((res) => {
            ...
            axios.put(`${API_BASE_URL}/project/${projectId}/task/${taskId}`, { title, description: desc })
                .then((res) => {
                    setAddTaskModal(false)
                        ...
                        >
                            <Dialog.Panel className="rounded-md bg-white w-6/12">
                                ...
                                <Dialog.Title as='div' className={'bg-white shadow px-6 py-4 rounded-t-md sticky top-0'}>
                                    ...
                                    <button onClick={() => setAddTaskModal(false)} className='absolute right-6 top-4 text-gray-500 hover:bg-gray-100 rounded focus:outline-none focus:ring focus:ring-offset-1 focus:ring-green-200 '>
